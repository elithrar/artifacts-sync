...
Before execution, the library confirms that each source ref still matches the event and reads the destination ref. The native-Git path checks the source again and uses compare-and-swap protection for forced updates and deletions. Stale deliveries become no-ops, matching destination refs suppress events generated by bidirectional synchronization, and divergent or concurrent destination changes fail instead of being overwritten.
